...
class Bot(DesktopBot):
    ...
    def process_nfp(self, file_nfp, master_all_path):
        ...
        # Carregar os dados do NFP a partir da linha 15
        nfp_df = pd.read_excel(file_nfp, sheet_name='Summy Daily_ByLine', header=14)

        # Encontrar a posição da coluna "BOM Date"
        if 'BOM Date' in nfp_df.columns:
            bom_date_index = nfp_df.columns.get_loc('BOM Date')
        else:
            print("Coluna 'BOM Date' não encontrada.")
            return

        # Selecionar as colunas usando índices (C, E e todas até antes da "BOM Date")
        colunas_selecionadas = [2, 4] + list(range(7, bom_date_index))
